...
const fetchData = async () => {
  ...
  const savedCalendarTasks = localStorage.getItem('commercial_calendar_tasks')
  if (savedCalendarTasks) {
    calendarTasks.value = JSON.parse(savedCalendarTasks)
  }
  const savedMissions = localStorage.getItem('commercial_missions')
  if (savedMissions) {
    boardMissions.value = JSON.parse(savedMissions)
  }
  isLoading.value = false
}

const saveCalendarTasks = () => {
  localStorage.setItem('commercial_calendar_tasks', JSON.stringify(calendarTasks.value))
}

const saveMissions = () => {
  localStorage.setItem('commercial_missions', JSON.stringify(boardMissions.value))
}
...
const addTask = async () => {
  ...
  if (table === 'calendar_tasks') {
    calendarTasks.value.push(newTaskObj)
    saveCalendarTasks()
  } else {
    boardMissions.value.push(newTaskObj)
    saveMissions()
  }
...
const deleteTask = async (id: number) => {
  ...
  if (table === 'calendar_tasks') {
    ...
    boardMissions.value = boardMissions.value.filter(t => t.id !== id)
    saveMissions()
  }
}
...
const saveTaskUpdate = async () => {
  ...
  if (table === 'calendar_tasks') {
    ...
    if (idx !== -1) calendarTasks.value[idx] = updatedTask
    saveCalendarTasks()
  } else {
    const idx = boardMissions.value.findIndex(t => t.id === editingTaskData.value.id)
    if (idx !== -1) boardMissions.value[idx] = updatedTask
    saveMissions()
  }
  
  selectedTaskForView.value = { ...updatedTask }
...
<template>
  <div class="flex-1 space-y-4 md:space-y-6 p-3 md:p-6 bg-slate-50/30 min-h-screen relative">
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 md:gap-6">
      <div>
        <h2 class="text-2xl font-black tracking-tight text-slate-900 uppercase">Planning Commercial</h2>
        <p class="text-muted-foreground text-[10px] font-black uppercase tracking-widest opacity-60">Gérez vos tâches et missions commerciales.</p>
      </div>
